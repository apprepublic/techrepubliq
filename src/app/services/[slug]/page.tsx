...
export default function ServicePage() {
  ...
  // Find the requested service or default to web-development
  const service = services.find((s) => s.slug === slug) ?? services[0];

  // All services for the nav
          ...
          <div className="lg:sticky lg:top-24">
            <CornerBracketFrame className="bg-accent-dim">
              ...
              <Button
                className="w-full"
                onClick={() =>
                  router.push(`/quote?category=${service.slug}`)
                }
              >
                Request a Quote
              </Button>
              <p className="mt-md text-xs text-slate text-center">
                Final price based on your specific requirements
              </p>
            </CornerBracketFrame>
          </div>
        </div>
