      ...
      <div className="flex flex-col lg:flex-row gap-xl">
            ...
            <div className="space-y-sm text-sm">
              ...
              {discountCode && (
                <div className="flex justify-between">
                  ...
                  <span className="font-mono text-success">{discountCode}</span>
                </div>
              )}
              <div className="border-t border-line pt-sm flex justify-between font-medium">
                <span className="text-ink">Total</span>
                <span className="font-mono text-ink text-lg">
                  {isNigeria ? "₦" : "$"}{isNigeria ? (amount * 1500).toLocaleString() : amount.toLocaleString()}
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="flex-1 max-w-[440px]">
          <div className="border border-line rounded-sm p-lg">
            ...
            {!isNigeria && (
              <button
                onClick={() => setGeoOverride(true)}
                className="block mb-md text-xs text-accent hover:text-accent-hover underline"
              >
            ...
            <div className="border border-line rounded-sm p-md mb-lg text-sm text-slate">
              <p className="mb-sm font-medium text-ink">
                Card details
              </p>
              <div className="space-y-sm">
                <div className="h-10 bg-paper border border-line rounded-sm px-sm flex items-center text-slate">
                  Card number
                </div>
                <div className="flex gap-sm">
                  <div className="h-10 bg-paper border border-line rounded-sm px-sm flex items-center text-slate flex-1">
                    MM/YY
                  </div>
                  <div className="h-10 bg-paper border border-line rounded-sm px-sm flex items-center text-slate flex-1">
                    CVC
            ...
            <Button
              ...
              onClick={handlePay}
            >
              Pay {isNigeria ? "₦" : "$"}{isNigeria ? (amount * 1500).toLocaleString() : amount.toLocaleString()}
            </Button>
          </div>
        </div>
      </div>
